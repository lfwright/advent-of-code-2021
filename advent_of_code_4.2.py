...
def find_first_winning_bingo_card_score(bingo_cards, numbers_called):
...
def find_last_winning_bingo_card_score(bingo_cards, numbers_called):
    number_called_check_dict = dict()
    for index, number_called in enumerate(numbers_called):
        number_called_check_dict[number_called] = index
        ...
        for bingo_card in bingo_cards:
            if bingo_card.check_table_against_calls(number_called_check_dict):
                if len(bingo_cards) == 1: #last winning card
                    last_winning_board = bingo_card
                    return (last_winning_board.total_unmarked_numbers(number_called_check_dict)) * number_called
...
if __name__ == '__main__':
    ...
    winning_card_score = find_last_winning_bingo_card_score(bingo_cards, numbers_called)
