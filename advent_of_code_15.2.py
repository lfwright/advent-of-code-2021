...
def extend_array(risk_array):
    risk_array_x_extend = risk_array
    for i in range(1,5):
        risk_array_x_extend = np.append(risk_array_x_extend, risk_array+np.full(risk_array.shape, i, dtype=int), 0)

    risk_array_extend = risk_array_x_extend
    for j in range(1,5):
        risk_array_extend = np.append(risk_array_extend, risk_array_x_extend+np.full(risk_array_x_extend.shape, j, dtype=int), 1)

    for idx, x in np.ndenumerate(risk_array_extend):
        risk_array_extend[idx] = ((x-1) % 9) + 1

    return risk_array_extend
...
if __name__ == "__main__":
    ...
    #risk_int_strips = [extend_risk_strip(strip) for strip in risk_int_strips]
    risk_array = load_into_array(risk_int_strips)

    risk_array = extend_array(risk_array)
